...
本程序使用**相对路径**读取数据文件，具体来说：

- 所有数据文件默认从"**当前工作目录下的 `data` 文件夹**"读取
- 只要 `DS_AI_Quiz.exe` 和 `data` 文件夹在同一级目录，并且在该目录中运行程序，就能正常找到所有文件
...
# 此时工作目录是 C:\Users\YourName，找不到 data 文件夹
```
...
```bash
# 先切换到程序目录
C:\Users\YourName> cd D:\Projects\DS_AI_Quiz
D:\Projects\DS_AI_Quiz> .\DS_AI_Quiz.exe
# 或者直接在该目录中双击运行
```
